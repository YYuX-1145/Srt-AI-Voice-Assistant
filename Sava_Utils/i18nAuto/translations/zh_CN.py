i18n_dict = {
    ...
    "Requires a restart": "需要重启",
    "Enable LAN access.": "开启局域网访问",
    ...
    "Auto Speech Speed": "自动语速",
    "Maximum audio acceleration ratio (requires ffmpeg)": "音频最大加速倍率，尝试加速音频以和起止时间同步。要求安装ffmpeg",
    "Minimum audio slowdown ratio (requires ffmpeg)": "音频最低慢放倍率，尝试拉伸音频以和起止时间同步。要求安装ffmpeg",
    "Loudness Normalization (requires ffmpeg)": "响度归一化(要求安装ffmpeg)",
    "Target Loudness(db)": "目标响度(db)",
    ...
    "Dynamic Mode": "动态模式",
    "Static Threshold(db)": "静态响度检测阈值",
    'Dynamic Threshold Ratio': "动态响度检测比率",
    ...
    "(ms)Minimum slice interval": "(ms)最短切割间隔",
    "(ms)Maximum silence length": "(ms)切完后静音最多留多长",
    "Other Parameters": "其他参数",
    "Start": "开始",
    "Stop": "停止",
