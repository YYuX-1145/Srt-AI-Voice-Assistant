...
| `api_launcher()`                     | （可选）用于创建触发调用的按钮控件。                        |
| `_UI()`                              | 必须实现，构建 UI 界面布局。你无需定义生成按钮`self.btn`。 **必须返回包含gradio组件的列表作为参数输入。**                          |
---
当然，以下是补充后的 **基类说明部分** 中关于 `Traducteur` 翻译器基类的详细介绍，格式与前文保持一致：
...
| `api(tasks, target_lang, interrupt_flag, *args, file_name="", **kwargs)` | 必须实现。处理翻译逻辑，接收 `construct_tasks()` 返回的任务列表，并返回翻译后的字符串列表（或附加消息）。 |
...
2. **注册**
   将所有 `Shared_Option` 对象以列表形式返回给 `register_settings()`，框架会自动生成对应的 Gradio 表单控件，并把用户输入写入全局 `Settings`。
