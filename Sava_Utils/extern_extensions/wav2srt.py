            ...
            with gr.TabItem("音频转字幕"):
                        with gr.Row():
                            ...
                            with gr.Column():
                                ...
                                self.wav2srt_pydir=gr.Textbox(value='auto',label="Python解释器路径",interactive=True)
...
`--hop_size` 默认:20 怎么算音量曲线，越小精度越大计算量越高（不是精度越大效果越好）<br>
                                ...
                                self.wav2srt_send2main.click(lambda x:send(x),inputs=[self.wav2srt_output],outputs=[file_main])
                                self.wav2srt_send2tr=gr.Button(value="发送到翻译",variant="secondary",interactive=True)
                                self.wav2srt_send2tr.click(lambda x:send(x),inputs=[self.wav2srt_output],outputs=[file_tr])
...
def send(fp_list):
     return [i.name for i in fp_list]